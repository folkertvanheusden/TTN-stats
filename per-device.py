...
c.execute('select nwkaddr, avg(rssi) as a_rssi, min(rssi) as min_rssi, max(rssi) as max_rssi, avg(lsnr) as a_lsnr, avg(size) as a_size, stddev(size) as sd_size, count(*) as n_msgs, (nwkaddr >> 1) & 127 as nwkid from rxpk group by nwkaddr')
